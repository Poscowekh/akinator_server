...
def bin_search(values: list, item) -> int:
    first = 0
    last = len(values) - 1
    pos = -1
    found = False
    ...
    while first<=last and not found:
        ...
        if values[midpoint][0] == item:
            pos = midpoint
            found = True
        else:
            if item < values[midpoint][0]:
                last = midpoint-1
            else:
                first = midpoint+1
...
def apply_update(new_data: dict):
    ...
    for theme, update in new_data:
        if theme_db.theme_exists(theme) == -1:
            theme_db.create_new_theme(theme, Version(1, 0))
        else:
        ...
        server_db = Connection(Connection.Type.server, theme, theme_db.latest_version(theme))

        server_db.update_base_ratings(new_data[theme]["pop_changes"])
        server_db.update_answers(new_data[theme]["mod_answers"])

        entity_ids = list()
        for name, desc in new_data["new_entities"]:
            entity_ids.append((name, server_db.insert_new_entity(name, desc)))

        question_ids = list()
        for text in new_data["new_questions"]:
            question_ids.append((text, server_db.insert_new_question(text)))

        for name, text, answer_value in new_data["new_answers"]:
            entity_id = bin_search(entity_ids, name)
            question_id = bin_search(question_ids, text)
